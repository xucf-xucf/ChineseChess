...
function qryGameById(gameID){
	...
	if(json.status == "0"){
		var moveJson = {};
		$.each(json.gameList,function(i,n){
			...
			moveArray[i] =JSON.parse(JSON.stringify(moveJson)); 
		});
	}else{
		$("#error3").val(json.desc);
	}
	CCEncode = JSON.parse(JSON.stringify(json.CCEncode)); 
	map = JSON.parse(JSON.stringify(json.gameMap)); 
