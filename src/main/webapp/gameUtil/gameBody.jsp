...
//被选中的时候
var onChoseChess = 0;
var chessLayout = false;
function layoutClick(){
	chessLayout = !chessLayout;
}
function choseChessClick(chessNum){
	onChoseChess = chessNum;
}
function useInitial(){
	putDefNone();
	putDef();
...
function deepClone(initalObj) {//利用json协助，搞定深度拷贝
    var obj = {};
...
function move(y,x,j,i,eat){
    ...
    if(isFirst){//打谱的时候第一次走棋，则默认棋盘绑定
    	msgBody.beginMap = deepClone(map);
    	isFirst = false;
    }
	var old = map[y][x];
	var _new = map[j][i];
