...
function move(y,x,j,i,eat){
    ...
    if(sureReMove == 1){//如果有悔棋消息则，退步
    	...
    	map[j][i] =  _new;
    	$("#CS"+j+"-"+i).html(
                "<section class='C "+calTemp+"' style='transform:translate("+(x-i)*45+"px,"+(y-j)*45+"px);'>"+temp+"</section>"
        )
...
function sendMsg(T,y,x,j,i,eat){
	var moveBody = new Object();
	moveBody.chess = T[0];
	moveBody.color = T[1].substring(1);
	moveBody.beforeY = y;
	moveBody.beforeX = x;
	moveBody.afterY = j;
	moveBody.afterX = i;
	...
	moveBody.map = map;
	moveBody.eat = eat;
	moveBody.sessionId = localSessionId;
	var json = JSON.stringify(moveBody); 
	send(json);
};
...
websocket.onmessage = function(event) {
	...
	if(json.rivalSessionId !=null){//对家id
		rivalSessionId = json.rivalSessionId;
	}
	if(json.afterX != null){ //有走子消息
		console.log(json.afterX != null);
		runNow = false;//对家走子
		 //move(beforeY,beforeX,json.afterY ,json.afterX ,eat);
	}
	if(json.responseReMove == "1"){//悔棋操作
	    ...
	    var T=getCText(y,x);
	    $("#CS"+y+"-"+x).html(
	            "<section class='C "+T[1]+"'>"+T[0]+"</section>"
	    )
		map[j][i] = m[j][i];
	    var T2=getCText(j,i);
	    $("#CS"+j+"-"+i).html(
	            "<section class='C "+T2[1]+"'>"+T2[0]+"</section>"
	    )
	}
